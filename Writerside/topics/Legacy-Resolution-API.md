...
| `KaCallInfo.singleCallOrNull` / `singleFunctionCallOrNull` / `singleVariableAccessCall` / `singleConstructorCallOrNull`                 | `tryResolveCall()?.single?.function` (or `?.variable` / `?.constructor`).                                                                                |
| `KaCallInfo.successfulCallOrNull` / `successfulFunctionCallOrNull` / `successfulVariableAccessCall` / `successfulConstructorCallOrNull` | `tryResolveCall()?.successful?.function` (or `?.variable` / `?.constructor`), or use the specialized `resolveSuccessfulCall()` on the concrete PSI type. |
| `KaCall`, `KaCallableMemberCall`                                                                                                        | [](KaSimpleOrMultiCall.md) &rarr; [](KaSimpleCall.md) / [](KaMultiCall.md).                                                                              |
...
| `KaCallableMemberCall.partiallyAppliedSymbol.signature` / `dispatchReceiver` / `extensionReceiver` / `contextArguments`                 | Inline accessors on `KaSimpleCall`: `signature`, `dispatchReceiver`, `extensionReceiver`, `contextArguments`, `typeArgumentsMapping`.                    |
...
| `KtElement.resolveToCall(): KaCallInfo?`                                                                                                | Specialized `resolveSuccessfulCall()` per PSI type, or `(element as? KtResolvableCall)?.resolveSuccessfulCall()`.                                        |
