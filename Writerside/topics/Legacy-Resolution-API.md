...
| `KaCallInfo.successfulCallOrNull` / `successfulFunctionCallOrNull` / `successfulVariableAccessCall` / `successfulConstructorCallOrNull` | `tryResolveCall()?.successful as? T`, or use the specialized `resolveSuccessfulCall()` on the concrete PSI type.                      |
