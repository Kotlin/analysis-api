...
| `KaCallInfo.successfulCallOrNull` / `successfulFunctionCallOrNull` / `successfulVariableAccessCall` / `successfulConstructorCallOrNull` | `tryResolveCall()?.successful as? T`, or use the specialized `resolveCall()` on the concrete PSI type.                                |
